...
class GameEngine:
    ...
    def resolve_system_combat(self, system_id: str, incoming_starfleets: List[Starfleet]):
        ...
        # Calculate attacking forces by player from incoming (moving) fleets
        attacker_forces = {}
        for starfleet in incoming_starfleets:
            owner = starfleet.owner
        ...
        # Include support-only attackers: any player (not defender_owner) whose
        # fleets are issuing a support order targeting this system, even if they
        # sent no fleet into the system this turn. This ensures 3-way fights
        # list every participant and their support contribution is counted.
        for starfleet in self.starfleets.values():
            if not (starfleet.orders and starfleet.orders.get("type") == "support"):
                continue
            if starfleet.orders.get("target") != system_id:
                continue
            owner = starfleet.owner
            if owner == defender_owner:
                continue  # supporting defender, handled below
            if owner not in attacker_forces:
                attacker_forces[owner] = {"starfleets": [], "support": 0}
        ...
        # Add starport defense if present and not sabotaged (controlled only)
        sabotaged = getattr(target_system, 'sabotaged_upgrades', [])
        if defender_owner and "starport" in target_system.upgrades and "starport" not in sabotaged:
            starport_bonus = 1
            defender_strength += 1
        ...
        # Calculate per-attacker strengths and totals; track tie for max
        attacker_strength_by_owner = {
            owner: len(forces["starfleets"]) + forces["support"]
            for owner, forces in attacker_forces.items()
        }
        total_attacker_strength = sum(attacker_strength_by_owner.values())
        # Only owners who actually moved a fleet into the system can claim it;
        # pure supporters contribute strength but cannot occupy.
        candidate_owners = {
            owner: attacker_strength_by_owner[owner]
            for owner, forces in attacker_forces.items()
            if len(forces["starfleets"]) > 0
        }
        max_attacker_strength = max(candidate_owners.values()) if candidate_owners else 0
        top_attackers = [o for o, s in candidate_owners.items() if s == max_attacker_strength and s > 0]
        strongest_attacker = top_attackers[0] if len(top_attackers) == 1 else None

        # Create detailed combat report (with breakdown for clarity)
        combat_report = {
            "system": target_system.name,
            "turn": self.current_turn,
            "attackers": dict(attacker_strength_by_owner),
            "attacker_breakdown": {
                owner: {
                    "fleets": len(forces["starfleets"]),
                    "support": forces["support"],
                    "total": len(forces["starfleets"]) + forces["support"],
                }
                for owner, forces in attacker_forces.items()
            },
            ...
            "defender_breakdown": {
                "fleets": len(defending_starfleets),
                "support": defender_support,
                "starport": starport_bonus,
                "total": defender_strength,
            },
            "outcome": None,
            "winner": None,
            "casualties": {"attackers": [], "defenders": []}
        ...
        if total_attacker_strength > defender_strength and strongest_attacker is not None:
            # Attackers win, a single strongest attacker takes the system
            combat_report["outcome"] = "attacker_victory"
            combat_report["winner"] = strongest_attacker
            ...
            # Move strongest attacker's starfleets in; others retreat
            for starfleet in incoming_starfleets:
                if starfleet.owner == strongest_attacker:
                    old_system = self.systems[starfleet.system_id]
                    del old_system.starfleets[starfleet.id]
        ...
        elif total_attacker_strength > defender_strength and strongest_attacker is None:
            # Attackers combined defeat defenders but two or more attackers tie
            # for max strength → contested. Defenders destroyed, system becomes
            # uncontrolled, all attackers bounce back (retreat).
            combat_report["outcome"] = "contested"
            combat_report["winner"] = None
            combat_report["contested_between"] = list(top_attackers)
            ...
            combat_report["winner"] = defender_owner
            for starfleet in incoming_starfleets:
